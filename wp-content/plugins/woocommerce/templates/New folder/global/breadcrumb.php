...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . __( '<div style="font-family: Gadugi, Arial;font-size:11px;margin-top:3px;">Search results for &ldquo;', 'woocommerce</div>' ) . get_search_query() . '&rdquo;' . $after;
